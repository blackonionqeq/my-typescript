...
enum TokenKind {
  Keyword, Identifier, StringLiteral, Seperator, Operator, EOF,
}
interface Token {
  kind: TokenKind
  text: string
...
class Tokenizer {
  ...
  private getNextToken(): Token {
    ...
    if (this.code.isEof()) return { kind: TokenKind.EOF, text:'' }
      ...
      } else if (c === '"') { return this.parseStringLiteral() } else if (c === '/') {
        this.code.next()
  ...
  parseStringLiteral() {
    ...
    if (this.code.peek() === '"')  token.text += this.code.next()
    return token
  }
...
abstract class Statement extends AstNode {
  static isStatement(node: any): node is Statement {
    if (!node) return false
    if (Object.getPrototypeOf(node) === Statement.prototype) return true
    return false
  }
}
...
class FunctionDecl extends Statement {
  constructor(public name: string, public body: FunctionBody) {
    super()
  }
  dump(prefix: string) {
...
class FunctionBody extends AstNode {
  constructor(public stmts: FunctionCall[]) {
    super()
  }
  dump(prefix: string) {
  ...
  static isFunctionBodyNode(node: any): node is FunctionBody {
    if (!node) return false
    if (Object.getPrototypeOf(node) === FunctionBody.prototype) return true
    return false
  }
}
...
class FunctionCall extends Statement {
  ...
  static isFunctionCallNode(node: any): node is FunctionCall {
    if (!node) return false
    if (Object.getPrototypeOf(node) === FunctionCall.prototype) return true
    return false
  }
  dump(prefix: string) {
...
class Parser {
  ...
  parseFunctionCall() {
    ...
    if (kind === TokenKind.Identifier) {
      ...
      if (t1.text === '(') {
        let t2 = this.tokenizer.next()
        while (t2.text !== ')') {
          if (t2.kind === TokenKind.StringLiteral) { params.push(t2.text) } else {
        ...
        t2 = this.tokenizer.next()
        if (t2.text === ';') return new FunctionCall(text, params)
  ...
  parseFunctionBody() {
    ...
    if (text === '{') {
      ...
      if (text === '}') return new FunctionBody(stmts)
      else {
...
abstract class AstVisitor {
  visitFunctionCall(fc: FunctionCall) {}
  visitFunctionBody(fb:FunctionBody) {
    let retVal
  ...
  visitFunctionDecl(fd: FunctionDecl) {
...
class RefResolver extends AstVisitor {
  ...
  visitProg(prog: Prog) {
    ...
    for (const x of prog.stmts) {
      ...
      if (typeof functionCall.parameters === 'object') this.resolveFunctionCall(prog, functionCall)
      else this.visitFunctionDecl(x as FunctionDecl)
    }
  }
  visitFunctionBody(functionBody: FunctionBody) {
    if (this.prog !== null) for (const x of functionBody.stmts) this.resolveFunctionCall(this.prog, x)
  }
  resolveFunctionCall(prog: Prog, functionCall: FunctionCall) {
    const functionDecl = this.findFunctionDecl(prog, functionCall.name)
    if (functionDecl !== null) functionCall.definition = functionDecl
    else if (functionCall.name !== 'println') {
      console.error("Error: cannot find definition of function " + functionCall.name);
    }
  }
  findFunctionDecl(prog: Prog, name: string) {
    for (const x of prog.stmts) {
      const functionDecl = x as FunctionDecl
      if (typeof functionDecl.body === 'object' && functionDecl.name === name) return functionDecl
    }
    return null
  }
}
...
class Interpretor extends AstVisitor {
  runFunction(functionCall: FunctionCall) {
    if (functionCall.name === 'println') {
      if (functionCall.parameters.length>0) console.log(...functionCall.parameters)
      return 0
